    ...
    <div className="btnserd">{btnColr ? <Button variant="outline-secondary" onClick={() => setBtnColr(!btnColr)}>♡
      ...
      <Card.Body>
        ...
        <Card.Text style={{fontSize: 14}} className="TextCard">
          ...
          <div>{colr ? <Button variant="outline-secondary" onClick={() => clickAddButton({id: props.id, name: props.name , cost: props.cost , img: props.img})}>+</Button> : <Button variant="success" onClick={() => setColr(!colr)}>✓</Button>}</div>
        </Card.Text>
